...
привязки упражнений лежат в localStorage.
...
подставляет туда имя репозитория. Локальная сборка и Netlify работают из корня без настройки.

**Netlify** — конфигурация в [../netlify.toml](../netlify.toml): команда сборки, папка публикации и
SPA-редирект.
